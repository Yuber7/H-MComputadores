...
class DetalleCompras extends BasicModel
{
    ...
    public function __construct($arrDetalleCompras = array())
    {
        ...
        $this->producto_id = !empty($arrDetalleCompras['producto_id']) ? Productos::searchForId($arrDetalleVentas['producto_id']) : new Productos();
        $this->compra_id = !empty($arrDetalleCompras['compra_id']) ? Compras::searchForId($arrDetalleVentas['venta_id']) : new Ventas();
     ...
     */
    public function getCompraId(): Ventas
    {
        return $this->compra_id;
    }

    /**
     * @param \App\Models\Ventas|mixed $compra_id
     */
    public function setVentaId(Compras $compra_id): void
    {
    ...
    {
        $result = $this->updateRow("UPDATE `h&mcomputadores`.detalle_compras SET valor_unitario = ?, cantidad = ?, producto_id= ?, compra_id = ?, estado = ? WHERE id = ?", array(
                $this->valor_unitario,
                $this->cantidad,
                $this->producto_id->getId(),
                $this->compra_id->getId(),
    ...
    public static function search($query): array
    {
        ...
        foreach ($getrows as $deta) {
            ...
            $DetalleCompras->compra_id = Compras::searchForId($deta['compra_id']);
    ...
    public static function searchForId($id)
    {
        ...
        if ($id > 0) {
            ...
            $DetalleCompras->compra_id = Compras::searchForId($getrow['compra_id']);
    ...
    static function DetalleCompraRegistrada(string $id){
        $result = DetalleCompras::search("SELECT * FROM `h&mcomputadores`.detalle_compra where id = " .$id);
        if ( count ($result) > 0 ) {
            return true;
        } else {
            return false;
    ...
    {
        return DetalleCompras::search("SELECT * FROM `h&mcomputadores`.detalle_compra");
    }
